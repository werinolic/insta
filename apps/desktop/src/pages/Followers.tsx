...
function FollowersList({ username }: { username: string }) {
  ...
  const { data, isLoading } = trpc.users.followers.useQuery({ username });
  ...
  if (items.length === 0) {
    return <p className="text-center text-gray-500 py-12 text-sm">No followers yet.</p>;
  }
      ...
      <div className="scrollable h-full">
        <div className="max-w-lg mx-auto">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200">
            ...
            <h1 className="font-semibold">{username} · Followers</h1>
          </div>
