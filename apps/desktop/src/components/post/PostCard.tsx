...
import { LikeButton, LikeButtonLive } from './LikeButton';
  ...
  return (
    <article className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center gap-3 p-3">
        <button onClick={() => navigate(`/${post.username}`)} className="flex-shrink-0">
          {post.avatarUrl ? (
      ...
      <div className="px-3 pt-2 pb-1 flex items-center gap-4">
        ...
        <Link to={`/p/${post.id}`} className="ml-auto text-xs text-gray-400 hover:text-gray-600">
          View post
        </Link>
      </div>
