...
function NewConversationPanel({
  ...
  const createConvo = trpc.conversations.create.useMutation({
    onSuccess: async (result, vars) => {
      ...
      onCreated(result.conversationId, vars.memberUsernames[0]);
    },
  });
    ...
    <div className="flex flex-col h-full">
      <div className="px-3 py-2 border-b border-gray-200 flex items-center gap-2">
        ...
        <input
          ...
          className="flex-1 text-sm bg-gray-100 rounded-full px-3 py-1.5 focus:outline-none"
        />
      </div>
      <div className="flex-1 overflow-y-auto">
        ...
        {(searchResults ?? []).map((u) => (
          <button
            key={u.id}
            onClick={() => createConvo.mutate({ memberUsernames: [u.username] })}
