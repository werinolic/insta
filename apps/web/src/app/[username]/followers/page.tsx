    ...
    <AuthGuard>
      ...
      <main className="pt-14">
        <div className="max-w-sm mx-auto px-4 py-6">
          <h1 className="text-base font-semibold mb-4">Followers</h1>
...
function FollowersList({ username }: { username: string }) {
  const { data, isLoading } = trpc.users.followers.useQuery({ username, limit: 50 });
  ...
  if (!data?.items.length) {
    return <p className="text-sm text-gray-500 text-center py-8">No followers yet.</p>;
  }
